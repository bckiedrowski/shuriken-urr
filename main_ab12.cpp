...
int main() {
  ...
  auto capture  = std::make_shared<mc::estimator>(
    "capture",      1, mc::estimator_type_t::TRACK_LENGTH_FLUX, std::nullopt, 40090, xs::endf_mt_t::Z_GAMMA, std::nullopt );
  std::vector< std::shared_ptr< mc::estimator > > estimators( { leakage, capture } );
  ...
  while ( ! mc_calc->is_done() ) {
    ...
    for ( auto history = 0 ; history < batch_size ; ++history ) {
       ...
       while ( p.alive() ) {
         ...
         //special logic for analytical benchmarks 1 and 2 to kill particle after one collision
         p.kill();
       }
       mc_calc->end_history();
    }
  ...
  xs[40090]->urr_data()->analytical_benchmark_1( 4.3675e-2, 10.0 );
  xs[40090]->urr_data()->analytical_benchmark_2( 4.3675e-2, 10.0 );
