#ifndef _MC_DOS_ESTIMATOR_HEADER_
#define _MC_DOS_ESTIMATOR_HEADER_
...
namespace mc {
  ...
  struct dos_profile_t {
    std::string                            name;
    std::optional< std::set<uint32_t> >    cell_id;
    std::optional< uint32_t >              zaid;
    std::optional< xs::endf_mt_t >         mt;
    std::optional< std::vector< double > > egrid;

    dos_profile_t( const std::string name,
                   const std::optional< std::set<uint32_t> > cell_id, const std::optional<uint32_t> zaid,
                   const std::optional<xs::endf_mt_t> mt,
                   const std::optional< std::vector< double > > egrid ) :
      name(name), cell_id(cell_id), zaid(zaid), mt(mt), egrid(egrid) {};
  };
  ...
  class dosEstimator {
    ...
    private:
      ...
      std::vector< double > _pert_rxn_xs_cache;
    ...
    public:
      dosEstimator( const std::vector< dos_profile_t > parameters,
                    const std::optional< rxn_ratio_vector > rxn_ratios,
                    const mc::mc_calc_t calc_mode );
//      dosEstimator( const uint32_t bank_size,
//                    const std::optional< uint32_t >      cell_id,
